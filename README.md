...
이 봇은 봇과의 채팅 및 각 그룹 채팅 모두에서 별도의 데이터로 관리되고 있습니다. 각 방에서 처음으로 /start를 수행한 사람이 추가, 상환 등의 작업이 가능합니다.

모든 기록은 debt.dat 및 state.dat에 저장되어 관리되고 있습니다. 프로그램을 종료한 이후에도, 다시 실행할 때 그 파일에서 자료를 받아오므로, 활용에 유의하시기 바랍니다.

기존에 사용하시던 debt.dat 및 state.dat는 그대로 활용하셔야 합니다. 그렇지 않으면, 지금까지 기록했던 자료들이 전부 소실됩니다.
(단, 1.2 버전 이하의 자료는 사용이 불가능합니다. 1.3부터 백업 절차를 수행해 주십시오.)
...
(스크립트를 사용하지 않고 Docker의 활용을 위해서는, 아래의 명령어를 수동으로 사용하셔야 합니다.)
    ...
    sudo docker run --name <프로세스 이름> -d -e TOKENKEY='<토큰값>' -v /etc/localtime:/etc/localtime <이미지 태그>
      ...
      예) sudo docker build --tag iskrobot:1.3 .

          sudo docker run --name iskrobot-1-3 -d -e TOKENKEY='12345678:A1B2C3D4E5F6G7H8i9_j10k11' -v /etc/localtime:/etc/localtime iskrobot:1.3
